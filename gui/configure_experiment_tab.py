    ...
    def run_experiment(self):
        ...
        if self.subset_warning_checkbox.isChecked():
            ...
            if will_not_run != '':
                ...
                if not okay :return
        if not self.save_changes_dialog(): return
...
class SubjectsTable(QtWidgets.QTableWidget):
    ...
    def update_subjects(self):
        ...
        self.subjects = [str(self.item(s, 2).text())
                         for s in range(self.num_subjects) if self.item(s, 2)]
...
class VariablesTable(QtWidgets.QTableWidget):
    ...
    def task_changed(self, task):
        ...
        self.variable_names = list(set([v_name for v_name in
            re.findall(pattern, file_content) if not v_name.endswith("___") and v_name != "custom_variables_dialog" and not v_name.startswith("hw_") and not v_name.startswith("btn_")]))
