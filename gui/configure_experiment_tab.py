...
class Configure_experiment_tab(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.experiment_parameters_layout.addWidget(
            self.hardware_test_label, 0, 1, QtCore.Qt.AlignmentFlag.AlignCenter
        )
        self.experiment_parameters_layout.addWidget(
            self.data_dir_label, 0, 2, 1, 2, QtCore.Qt.AlignmentFlag.AlignCenter
        )
    ...
    def load_experiment(self, experiment_name):
        ...
        if "subset_warning" in experiment.keys():  # New style experiment file.
            ...
            self.subset_warning_checkbox.setChecked(True)
            subjects_dict = {
                subject: {"run": True, "setup": setup} for setup, subject in experiment["subjects"].items()
            }
...
class SubjectsTable(QtWidgets.QTableWidget):
    ...
    def update_available_setups(self, i=None):
        ...
        selected_setups = set([str(self.cellWidget(s, 1).currentText()) for s in range(self.n_subjects)])
        self.available_setups = sorted(list(self.all_setups))
    ...
    def update_subjects(self):
        ...
        self.subjects = [str(self.item(s, 2).text()) for s in range(self.n_subjects) if self.item(s, 2)]
...
class VariablesTable(QtWidgets.QTableWidget):
    ...
    def task_changed(self, task):
        ...
        self.variable_names = list(
            set([v_name for v_name in re.findall(pattern, file_content) if not v_name[-3:] == "___"])
        )
