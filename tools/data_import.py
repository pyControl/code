...
def session_dataframe(file_path, paired_events={}, pair_end_suffix=None):
    ...
    for line in all_lines:
        if line[0] == "I":  # Info line.
            ...
            line_dicts.append({"type": "info", "name": name, "value": value})
        elif line[0] == "D":  # Data line.
            timestamp, ID = [int(i) for i in line.split(" ")[1:]]
            line_dicts.append(
                {"type": "state" if ID in state_IDs.values() else "event", "name": ID2name[ID], "time": int(timestamp)}
            )
        elif line[0] == "P":  # Print line.
            line_dicts.append(
                {"type": "print", "time": int(line[2:].split(" ", 1)[0]), "value": line[2:].split(" ", 1)[1]}
            )
...
def experiment_dataframe(folder_path, paired_events={}, pair_end_suffix=None):
    ...
    for session_filename in session_filenames:
        ...
        session_df = session_dataframe(
            os.path.join(folder_path, session_filename), paired_events=paired_events, pair_end_suffix=pair_end_suffix
        )
