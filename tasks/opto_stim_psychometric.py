...
v.autoreward = False
# psychometic curve measurements  with a single 'reminder' power of 0.5mW

v.LED_currents = [3.73, 0.85, 0.7, 0.56, 0.41, 0.27]
...
v.tot_trials = 200 #total number of trials to do 

# trial parameters
v.chanceGo = 0.7
    
v.lick_window = 2  # reward time window during which mouse has to lick (s)
...
v.miss_switch = 6 # the number of consecutive missed trials before the animal is switched back to autoreward
...
def trial_start(event):
    ...
    if event == 'entry':
        ...
        if v.consecNoGo == 3 or v.isGo and v.consecGo < 3:
            ...
            print('goTrial')
            #need to choose this here due to pulsing
            v.LED_current = choice(v.LED_currents)
            print('LED_current is %s'%v.LED_current)
            v.isGo = True
            v.isNoGo = False
...
def ITI(event):
    ...
    if event == 'entry':
        ...
        print('hit_rate is %s'%sum(v.rolling_hit))
        print('fa_rate is %s'%sum(v.rolling_fa))
