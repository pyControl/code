...
class Data_logger:
    ...
    def set_state_machine(self, sm_info):
        ...
        self.ID2name_hw = {
            ai["ID"]: name for name, ai in self.sm_info["analog_inputs"].items()  # Dict mapping hardware IDs to names.
        }
    ...
    def data_to_string(self, new_data, verbose=False):
        ...
        for nd in new_data:
            if nd[0] == "D":  # State entry or event.
                ...
                if (
                    not verbose
                ):  # In data files multi-line tracebacks have ! prepended to all lines aid parsing data file.
    ...
    def save_analog_chunk(self, ID, sampling_rate, timestamp, data_array):
        ...
        if not self.analog_files[ID]:
            file_name = os.path.splitext(self.file_path)[0] + "_" + self.ID2name_hw[ID] + ".pca"
