...
class Digital_input(IO_object):
    ...
    def _publish_if_edge_has_event(self, timestamp):
        ...
        if self.pin_state and self.rising_event_ID:  # Rising edge.
            fw.event_queue.put((timestamp, fw.event_typ, "i", self.rising_event_ID))
        elif (not self.pin_state) and self.falling_event_ID:  # Falling edge.
            fw.event_queue.put((timestamp, fw.event_typ, "i", self.falling_event_ID))
...
class Analog_threshold(IO_object):
    ...
    def _process_interrupt(self):
        ...
        if self.crossing_direction:
            fw.event_queue.put((self.timestamp, fw.event_typ, "i", self.rising_event_ID))
        else:
            fw.event_queue.put((self.timestamp, fw.event_typ, "i", self.falling_event_ID))
...
class Rsync(IO_object):
    ...
    def _timer_callback(self):
        if self.state:  # Pin high -> low, set timer for next pulse.
            ...
            fw.data_output_queue.put((fw.current_time, fw.event_typ, "s", self.event_ID))
        self.state = not self.state
